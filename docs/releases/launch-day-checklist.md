...
- [ ] Confirmed the physical commit hash matches the targeted `vX.X.X` branch tag.
...
- [ ] Log in as Admin → Visit `/admin` → Must NOT redirect back to dashboard!

## 4. Supabase Checks (Database)
- [ ] Open the **Supabase Dashboard** → Authentication.
- [ ] Create a completely new dummy account.
...
- [ ] **Action:** Submit a physical $1 test charge or test-credit card.
...
2. Select the *previous* deployment block.
3. Click the explicit three-dot menu → **Redeploy**.
4. Check Supabase `.env` keys. A rollback usually means a key mismatch!

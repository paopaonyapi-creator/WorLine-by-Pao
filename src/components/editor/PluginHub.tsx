...
import {
  ...
  Earth, Lock, Satellite, Atom, Snowflake, Rocket, Layers, BatteryWarning, CheckCircle2
} from "lucide-react";
  ...
  const PluginItem = ({ icon: Icon, title, id, desc, status = "Installed" }: { icon: any; title: string; id: string; desc: string; status?: "Installed" | "Premium" | "Beta" }) => (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[10px] text-muted-foreground leading-tight mt-0.5 pr-4 line-clamp-2">{desc}</div>
      </div>
    </button>
  );
